...
class BrowserAutomator:
    ...
    def setup_driver(self):
        """
        Ses up Chrome or Edge WebDriver.
        """
        self.logger.info("Setting up WebDriver...Please be patient")

        try:
            self._setup_chrome()
        except Exception as chrome_error:
            self.logger.warning(f"Chrome WebDriver failed: {chrome_error}. Trying Edge.")

            try:
                self._setup_edge()
            except Exception as edge_error:
                self.logger.critical(f"Edge WebDriver failed: {edge_error}")
                raise RuntimeError("WebDriver setup failed.") from edge_error
